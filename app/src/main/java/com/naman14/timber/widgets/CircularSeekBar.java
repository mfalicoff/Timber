...
public class CircularSeekBar extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        float touchEventRadius = calculateTouchEventRadius(x, y);
        float[] radiusBounds = calculateRadiusBounds(touchEventRadius);
        float innerRadius = radiusBounds[0];
        float outerRadius = radiusBounds[1];
        float touchAngle = calculateTouchAngle(x, y);

        boolean result = false;
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                result = handleActionDown(touchEventRadius, innerRadius, outerRadius, touchAngle);
                break;
            case MotionEvent.ACTION_MOVE:
                result = handleActionMove(touchEventRadius, innerRadius, outerRadius, touchAngle);
                break;
            case MotionEvent.ACTION_UP:
                result = handleActionUp();
                break;
            case MotionEvent.ACTION_CANCEL:
                result = handleActionCancel();
                break;
        }
    ...
    private float calculateTouchEventRadius(float x, float y) {
        float distanceX = mCircleRectF.centerX() - x;
        float distanceY = mCircleRectF.centerY() - y;
        return (float) Math.sqrt(Math.pow(distanceX, 2) + Math.pow(distanceY, 2));
    }

    private float[] calculateRadiusBounds(float touchEventRadius) {
    ...
    private float calculateTouchAngle(float x, float y) {
        float touchAngle = (float) ((Math.atan2(y, x) / Math.PI * 180) % 360);
        return touchAngle < 0 ? 360 + touchAngle : touchAngle;
    }

    private boolean handleActionDown(float touchEventRadius, float innerRadius, float outerRadius, float touchAngle) {
    ...
    private boolean handleActionMove(float touchEventRadius, float innerRadius, float outerRadius, float touchAngle) {
        if (!mUserIsMovingPointer) {
            return false;
        }

        updateDirection(touchAngle);

        if (lockAtStart && mIsMovingCW || lockAtEnd && !mIsMovingCW) {
            handleLocking();
        } else {
            updateProgressAndRecalculate(touchAngle, touchEventRadius, outerRadius);
        }

        lastCWDistanceFromStart = calculateCwDistanceFromStart(touchAngle);
        invalidate();
        return true;
    }

    private boolean handleActionUp() {
        // Implementation for ACTION_UP event
        mPointerHaloPaint.setAlpha(mPointerAlpha);
        mPointerHaloPaint.setColor(mPointerHaloColor);
        if (mUserIsMovingPointer) {
            mUserIsMovingPointer = false;
    ...
    private boolean handleActionCancel() {
        // Implementation for ACTION_CANCEL event
        mPointerHaloPaint.setAlpha(mPointerAlpha);
        mPointerHaloPaint.setColor(mPointerHaloColor);
        mUserIsMovingPointer = false;
        invalidate();
